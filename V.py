...
import io, queue, hashlib, base64, smtplib, yaml
...
class GreeksMonitor:
    ...
    def calculate_portfolio_greeks(self, prices: Dict) -> Dict[str, float]:
        ...
        for leg in self.legs:
            ...
            if leg['role'] == 'CORE':
                gamma = 0.01 * qty
                theta = (leg.get('strike', 23500) / 365) * qty
                vega = (leg.get('strike', 23500) * 0.02) * qty
                total_gamma += gamma * multiplier
                total_theta += theta * multiplier
                total_vega += vega * multiplier
        greeks = {'delta': total_delta, 'gamma': total_gamma, 'theta': total_theta, 'vega': total_vega, 'timestamp': time.time()}
...
class PerformanceReporter:
    ...
    def generate_daily_report(self, report_date: date = None) -> str:
        ...
        try:
            ...
            winners = [t[3] for t in trades_detail if t[3] > 0]
            losers = [t[3] for t in trades_detail if t[3] < 0]
...
# ---------- DATABASE ----------
db_writer = DatabaseWriter()  # instantiate global
...
class StrategyFactory:
    ...
    def _calculate_pro_wing_width(self, spot: float, vol_metrics: VolMetrics, dte: int, interval: int) -> int:
        """
        Calculate wing width based on EXPECTED MOVE, not premium.
        Expected Move = Spot × (Annual Vol / 100) × √(DTE/365)
        """
    ...
    def execute_strategy(self, legs: List[Dict]) -> List[Dict]:
        ...
        if hedges:
            ...
            if len(hedge_results) != len(hedges):
                ...
                self._flatten_legs(hedge_results); return []
        ...
        if cores:
            ...
            if len(core_results) != len(cores):
                ...
                self._flatten_legs(hedge_results + core_results); return []
...
class RiskManager:
    ...
    def monitor(self):
        ...
        greeks_monitor = GreeksMonitor(self.legs)
        while self.running:
            try:
                ...
                # CONFIGURABLE WINDOW (Recommended: 2:15 PM - 3:15 PM)
                exit_start = dtime(14, 15)
                exit_end   = dtime(15, 15)
                ...
                # 2. SMART EXIT: Expiry-Minus-1 (1 DTE)
                    ...
                    # Late Exit (Missed Window) -> FORCE EXIT
                    ...
                    # Before Window -> HOLD (Log every 30 mins, anti-spam)
                ...
                # Greeks check
                greeks = greeks_monitor.calculate_portfolio_greeks(prices)
                warnings = greeks_monitor.check_greek_limits(greeks)
                if warnings:
            ...
            if holiday_response.status == 'success' and holiday_response.data:
                ...
                for holiday in holidays:
                    if hasattr(holiday, 'holiday_type') and holiday.holiday_type == 'TRADING_HOLIDAY':
                        ...
                        return FalseI notice the response was cut off. Let me continue with the complete code:
